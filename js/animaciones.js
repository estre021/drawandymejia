...
function animateAboutSection() {
  ...
  // Image scale animation on scroll (we keep this for a premium look, while fade-in is handled by the global system)
...
// Global scroll reveal system using GSAP ScrollTrigger
function initGlobalScrollReveal() {
  const targets = gsap.utils.toArray('.reveal-on-scroll');
  if (targets.length === 0) return;

  targets.forEach(el => {
    // Skip Hero section (#inicio) since it has its own load-in animations
    if (el.id === 'inicio') return;

    // 1. Calculate delay from classes (e.g. delay-3 -> 0.24s)
    let delay = 0;
    el.classList.forEach(className => {
      if (className.startsWith('delay-')) {
        const delayNum = parseInt(className.split('-')[1]);
        if (!isNaN(delayNum)) {
          delay = delayNum * 0.08; // 0.08s increment per delay step
        }
      }
    });
    ...
    // 2. Create GSAP scroll reveal trigger using gsap.to() to animate to the visible state
    gsap.to(el, {
      opacity: 1,
      x: 0,
      y: 0,
      ...
      scrollTrigger: {
        trigger: el,
        start: "top 88%", // Triggers when the top of the element hits 88% of viewport height
        toggleActions: "play none none none",
        onEnter: () => el.classList.add('revealed')
      },
      clearProps: "all" // Clear GSAP inline styles after completion so hover/responsive rules work
    });
  });
}
